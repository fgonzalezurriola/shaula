...
assert_failure_code ERR_OUTPUT_PATH_INVALID env SHAULA_COMPOSITOR=niri NIRI_SOCKET=/tmp/niri.sock WAYLAND_DISPLAY=wayland-1 ./zig-out/bin/shaula capture fullscreen --json --output /tmp/::invalid::/path.png
...
assert_failure_code ERR_UNKNOWN_UNMAPPED env SHAULA_INJECT_UNKNOWN_FAILURE=1 SHAULA_COMPOSITOR=niri NIRI_SOCKET=/tmp/niri.sock WAYLAND_DISPLAY=wayland-1 ./zig-out/bin/shaula capture fullscreen --json --no-preview
