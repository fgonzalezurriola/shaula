...
static void managed_block_result_clear(ManagedBlockResult *result) {
  g_clear_pointer(&result->path, g_free);
  g_clear_pointer(&result->backup_path, g_free);
}

static char *niri_config_path(const char *override) {
  if (override != NULL && override[0] != '\0')
    return g_strdup(override);
...
static gboolean install_managed_block(const char *path_override,
...
static GPtrArray *niri_keybind_conflicts(const char *path) {
...
static gboolean remove_managed_keybinds(const char *path_override,
                                        gboolean dry_run,
                                        ManagedBlockResult *result,
                                        gboolean *invalid) {
  *result = (ManagedBlockResult){0};
