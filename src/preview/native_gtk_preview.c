...
static gboolean detect_dark_theme(void) {
	...
	if (theme != NULL) {
		gboolean dark = (g_str_has_suffix(theme, "-dark") || g_str_has_suffix(theme, "-Dark"));
		g_free(theme);
		if (dark) return TRUE;
	}
	gint pref = 0;
	...
	if (pref) return TRUE;
	gchar *scheme = NULL;
	g_object_get(settings, "gtk-color-scheme", &scheme, NULL);
	if (scheme != NULL) {
		gboolean dark = (g_strstr_len(scheme, -1, "dark") != NULL);
		g_free(scheme);
		if (dark) return TRUE;
	}
	return FALSE;
...
static void update_theme_state(void) {
...
static void update_zoom_label(void) {
	if (state.zoom_label == NULL) return;
	...
	for (int y = 0; y < height; y += tile) {
		for (int x = 0; x < width; x += tile) {
			...
			if (state.is_dark) {
				...
				cairo_set_source_rgb(cr, alt ? 0.90 : 0.87, alt ? 0.90 : 0.87, alt ? 0.91 : 0.88);
			}
...
static GtkWidget *make_toolbar_button(const char *icon_name, const char *tooltip, GCallback callback) {
...
static void apply_toolbar_css(GtkWidget *widget) {
	...
	gtk_widget_add_css_class(widget, "shaula-toolbar");
}
...
static GtkWidget *build_toolbar(void) {
	...
	gtk_box_append(GTK_BOX(box), make_toolbar_button("edit-copy-symbolic", "Copy  (Ctrl+C)", G_CALLBACK(on_copy_clicked)));
	gtk_box_append(GTK_BOX(box), make_toolbar_button("document-save-as-symbolic", "Save As  (Ctrl+S)", G_CALLBACK(on_save_clicked)));
	gtk_box_append(GTK_BOX(box), make_separator());
	gtk_box_append(GTK_BOX(box), make_toolbar_button("zoom-fit-best-symbolic", "Fit  (F)", G_CALLBACK(on_fit_clicked)));
	gtk_box_append(GTK_BOX(box), make_toolbar_button("zoom-original-symbolic", "100%  (0)", G_CALLBACK(on_actual_clicked)));
	gtk_box_append(GTK_BOX(box), make_separator());

	state.zoom_label = gtk_label_new("100%");
	gtk_widget_add_css_class(state.zoom_label, "shaula-zoom-label");
	gtk_widget_set_valign(state.zoom_label, GTK_ALIGN_CENTER);
	gtk_box_append(GTK_BOX(box), state.zoom_label);

	gtk_box_append(GTK_BOX(box), make_separator());
	gtk_box_append(GTK_BOX(box), make_toolbar_button("edit-delete-symbolic", "Discard  (Esc)", G_CALLBACK(on_discard_clicked)));

	apply_toolbar_css(box);
	return box;
}
