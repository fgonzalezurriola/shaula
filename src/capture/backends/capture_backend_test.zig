...
test "implicit output path resolves under temporary captures directory" {
    ...
    switch (outcome) {
        .success => |success| {
            try std.testing.expect(std.mem.startsWith(u8, success.path, "/tmp/shaula-test-runtime/shaula/captures/"));
        },
...
test "saved output path falls back to HOME shaula when Pictures path is unusable" {
    ...
    switch (outcome) {
        .success => |success| {
            const expected_prefix = try std.fmt.allocPrint(std.testing.allocator, "{s}/shaula/", .{home});
