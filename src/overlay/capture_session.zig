...
pub const CaptureSession = struct {
    ...
    pub fn init(output: toolbar_layout.Rect, persisted_toolbar: ?toolbar_layout.Point) CaptureSession {
        return .{
            .output = output,
    ...
    pub fn updateSelection(self: *CaptureSession, geometry: toolbar_layout.SelectionRect) void {
...
test "capture session reuses persisted toolbar before selection" {
    const session = CaptureSession.init(defaultOutput(), .{ .x = 100, .y = 500 });
...
test "capture session updates toolbar from active selection" {
    var session = CaptureSession.init(defaultOutput(), null);
